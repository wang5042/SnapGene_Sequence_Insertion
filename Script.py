...
def snapgene_file_to_gbk(read_file_object, write_file_address):
    """Convert a file object."""
    def randomSelectColor():
        color = ["#e7effa","#96c37d","#f3d266","#f7d1e1","#f8f3f9","#9ac9db","#f1d77e","#9dc3e7","#b1ce46","#5f97d2","#ef7a6d","#9394e7"]
        import random
        chosen = random.randint(0,len(color)-1)
        return color[chosen]
    ...
    data = read_file_object
    write_file_object = open(write_file_address,'w')
...
has {} segments:'
            ...
            wfo.write("\n"+21 * " "+'/ApEinfo_fwdcolor="{}"'.format(randomSelectColor()))
            wfo.write("\n"+21 * " "+'/ApEinfo_revcolor = "{}'.format(randomSelectColor()))
            if strand == "-":
                wfo.write('; direction: LEFT"\n')
                # wfo.write('"\n')
            elif strand == "+":
...
if __name__ == "__main__":
    ...
    flag = True
    while(flag):
        read_file_address = str(input("输入。dna文件地址：(退出请输入q)"))
        if(read_file_address == "q"):
            break
        if os.path.isdir(read_file_address):
            print("bbbbb")
            files = os.listdir(read_file_address)
            for file in files:
                file_path = read_file_address+"/"+file
                if not os.path.isdir(file_path):
                    write_file_address = file_path[0:-4] + ".gbk"
                    print(read_file_address)
                    print(write_file_address)
                    snapgene_file_to_gbk(file_path, write_file_address)
        else:
            write_file_address = read_file_address[0:-4] + ".gbk"
            print(read_file_address)
            print(write_file_address)
            snapgene_file_to_gbk(read_file_address, write_file_address)
    os.system("pause")